...
function ViewResume(props) {
    ...
    // const candidateId = props.match.params.cid;
    ...
    const handleUpdatePass = async () => {
        await updatePass(cid, jid)
            .then(response => {
                Alert.success("Accepted this candidate")
    ...
    const handleUpdateDecline = async () => {
        await updateDecline(cid, jid)
            .then(response => {
                Alert.success("Send decline to candidate")
                //history.push(`/view-resume/${id}`);
            }).catch(error => {
                Alert.error((error) || ('something went wrong'));
            });
    ...
    const updateStatus = () => {
        const response = updateViewd(cid, jid)
            .then(res => {
    ...
    const fetchStatus = async () => {
        const result = await getJobApplyStatus(cid, jid);
        setStatus(result.status);
        if (result.status === 'pass' || result.status === 'decline') {
            setButton(true);
        }
        setLoading(false);
